...
// */
//
//package com.moilioncircle.replicator.cluster.gossip.backup;
//
...
//import com.moilioncircle.replicator.cluster.gossip.*;
...
//        Map.Entry<Long, ClusterNode> entry = new AbstractMap.SimpleEntry<Long, ClusterNode>(System.currentTimeMillis() + CLUSTER_BLACKLIST_TTL, node);
...
//
//                if (nodePFailed(link.node)) { //nodeTimedOut判断是否是CLUSTER_NODE_PFAIL
...
//            client.pauseClients(System.currentTimeMillis() + (CLUSTER_MF_TIMEOUT * 2));
...
//        if (server.cluster.mfEnd != 0 && client.clientsArePaused()) {
//            server.clientsPauseEndTime = 0;
//            client.clientsArePaused();
//        }
//        server.cluster.mfEnd = 0;
