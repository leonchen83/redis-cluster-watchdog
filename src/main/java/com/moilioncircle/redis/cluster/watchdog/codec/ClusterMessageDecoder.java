...
public class ClusterMessageDecoder extends ByteToMessageDecoder {
    ...
    protected ClusterMessage decode(ByteBuf in) {
        ...
        try {
            ...
            in.readBytes(sender);
            if (!Arrays.equals(sender, ClusterConstants.CLUSTER_NODE_NULL_NAME)) {
            ...
            if (!Arrays.equals(slaveof, ClusterConstants.CLUSTER_NODE_NULL_NAME)) {
                msg.slaveof = new String(slaveof);
            }
            byte[] myip = new byte[46];
            in.readBytes(myip);
            if (!Arrays.equals(myip, ClusterConstants.CLUSTER_NODE_NULL_IP)) {
            ...
            if (msg.type == ClusterConstants.CLUSTERMSG_TYPE_PING || msg.type == ClusterConstants.CLUSTERMSG_TYPE_PONG || msg.type == ClusterConstants.CLUSTERMSG_TYPE_MEET) {
                ...
                for (int i = 0; i < msg.count; i++) {
                    ...
                    if (!Arrays.equals(nodename, ClusterConstants.CLUSTER_NODE_NULL_NAME)) {
                    ...
                    byte[] ip = new byte[46];
                    in.readBytes(ip);
                    if (!Arrays.equals(ip, ClusterConstants.CLUSTER_NODE_NULL_IP)) {
            ...
            } else if (msg.type == ClusterConstants.CLUSTERMSG_TYPE_FAIL) {
                msg.data = new ClusterMessageData();
            ...
            } else if (msg.type == ClusterConstants.CLUSTERMSG_TYPE_PUBLISH) {
                msg.data = new ClusterMessageData();
            ...
            } else if (msg.type == ClusterConstants.CLUSTERMSG_TYPE_UPDATE) {
                msg.data = new ClusterMessageData();
