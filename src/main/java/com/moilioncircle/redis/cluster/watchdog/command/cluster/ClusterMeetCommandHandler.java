...
public class ClusterMeetCommandHandler extends AbstractCommandHandler {
    ...
    public void handle(Transport<byte[][]> t, String[] message, byte[][] rawMessage) {
        ...
        catch (Exception e) { replyError(t, "ERR Invalid port:" + message[3]); return; }

        if (message.length == 5) {
            try { busPort = parseInt(message[4]); }
            catch (Exception e) { replyError(t, "ERR Invalid bus port:" + message[4]); return; }
        ...
        if (port <= 0 || port > 65535) { replyError(t, "ERR Invalid port:" + port); return; }
        if (busPort <= 0 || busPort > 65535) { replyError(t, "ERR Invalid bus port:" + busPort); return; }
        if (message[2] == null || message[2].length() == 0) { replyError(t, "ERR Invalid address:" + message[2]); return; }
