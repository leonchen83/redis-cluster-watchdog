...
import static com.moilioncircle.redis.cluster.watchdog.ClusterConstants.CLUSTER_FAIL_REPORT_VALIDITY_MULTI;
...
import static com.moilioncircle.redis.cluster.watchdog.ClusterConstants.HEX_CHARS;
...
public class ClusterNodeManager {
    ...
    public ClusterNode createClusterNode(String name, int flags) {
        ...
        n.name = name == null ? getRandomHexChars() : name;
        n.flags = flags;
        return n;
    }
    ...
    public void clusterDelNode(ClusterNode node) {
        ...
        server.cluster.nodes.values().stream().filter(t).forEach(c);
    ...
    public boolean clusterNodeRemoveSlave(ClusterNode master, ClusterNode slave) {
        ...
        if (r && master.slaves.size() == 0) master.flags &= ~CLUSTER_NODE_MIGRATE_TO;
        return r;
    }
